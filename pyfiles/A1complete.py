...
print('Data split into 75% train and 25% validation')

print('Linear regression model is implemented and trained')
print('First the solver is optimised')
#Evaluate using 'lbfgs' solver
model = LogisticRegression(solver = 'lbfgs', max_iter=10000)
model.fit(X_train, Y_train)
result = model.score(X_test, Y_test)
print("Accuracy using 'lbfgs'solver: %.2f%%" % (result*100.0))

#Evaluate using 'newton-cg' solver
model = LogisticRegression(solver = 'newton-cg', max_iter=10000)
model.fit(X_train, Y_train)
result = model.score(X_test, Y_test)
print("Accuracy using 'netton-cg' solver: %.2f%%" % (result*100.0))

# Evaluate using 'liblinear' solver
model = LogisticRegression(solver = 'liblinear', max_iter=10000)
model.fit(X_train, Y_train)
result = model.score(X_test, Y_test)
print("Accuracy using 'liblinear' solver: %.2f%%" % (result*100.0))

print('lbfgs solver chosen and now C value is optimised')
# Evaluate using C=10
model = LogisticRegression(solver = 'lbfgs', max_iter=10000, C=10)
model.fit(X_train, Y_train)
result = model.score(X_test, Y_test)
print("Accuracy using C=10: %.2f%%" % (result*100.0))

# Evaluate using C=1.0
model = LogisticRegression(solver = 'lbfgs', max_iter=10000, C=1.0)
model.fit(X_train, Y_train)
result = model.score(X_test, Y_test)
print("Accuracy using C=1.0: %.2f%%" % (result*100.0))

# Evaluate using C=0.1
model = LogisticRegression(solver = 'lbfgs', max_iter=10000, C=0.1)
model.fit(X_train, Y_train)
result = model.score(X_test, Y_test)
print("Accuracy using C=0.1: %.2f%%" % (result*100.0))

# Evaluate using C=0.01
model = LogisticRegression(solver = 'lbfgs', max_iter=10000, C=0.01)
model.fit(X_train, Y_train)
result = model.score(X_test, Y_test)
print("Accuracy using C=0.01: %.2f%%" % (result*100.0))
print('C value of 0.01 chosen')
